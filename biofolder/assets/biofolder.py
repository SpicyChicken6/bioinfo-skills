...
# Keep in sync with task-methods.md; embed it so the installed helper is standalone.
TASK_METHODS_TEMPLATE = """# Methods: <task name>
...
def add_task(root, module, name):
    ...
    write_missing(
        task / "README.md",
        f"# {name}\n\n## Description\n\n## Plan\n\n"
        "See [methods and run summaries](agent/docs/methods.md).\n\n## Output\n",
    )
    ...
    write_missing(
        task / "agent/docs/methods.md",
        TASK_METHODS_TEMPLATE.replace("<task name>", name, 1),
    )
    return task, False
